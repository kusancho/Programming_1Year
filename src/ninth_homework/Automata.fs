...
let treeToArray2D (tree: extendedTree<_>) =
...
let seqToAtm (input: list<_>) algStruct =
    let mtx =
        ...
        mtx
    TreeNFA(HashSet(0), HashSet(input.Length), array2DToTree mtx algStruct)
...
(*let epsClosure (atm: TreeNFA<_>) =
    ...
    let monoid = new Monoid<_>(addSets, null)
    let semiRing = new SemiRing<_>(monoid, multSets)
    let eCls = extendedTree.transitiveClosure
